...
class RecogniseHandWrittenDigits():
	...
	def createOptimizer(self, opt):
		...
		if opt=="adam":
			...
			designed_optimizer = optimizers.SGD(lr_rate=0.1)
	...
	def trainModel(self, model):
		...
		# model.fit_generator(self.datagen.flow(self.x_train, self.y_train, batch_size=self.size_batches),
		...
		# self.trained_model.save("cnn_mnist_zca.h5")


		# # history = model.fit(self.datagen.flow(self.x_train,self.y_train,batch_size=self.size_batches), validation_data=(self.x_test, self.y_test), epochs=self.num_epochs, batch_size=self.size_batches)
		history = model.fit_generator(self.datagen.flow(self.x_train, self.y_train, batch_size=self.size_batches),
							steps_per_epoch = len(self.x_train)/self.size_batches, validation_data=(self.x_test, self.y_test), epochs=self.num_epochs)
		self.trained_model = model
	...
	def loadModel(self):
		...
		self.trained_model = keras.models.load_model("cnn_mnist_zca.h5")
